...
# ── 1. 수업 제목 ──
st.title("분자의 구조")  # ← 교과별 제목으로 자유롭게 수정하세요.
...
# ── 3‑1. 서술형 문제 1 표시 ──
QUESTION_1 = """
같은 족 내에서 원자번호가 증가할수록 전기음성도는 어떻게 변화하는지 서술하시오.
"""  # ← 교사가 원하는 서술형 문제로 변경
...
# ── 3‑2. 서술형 문제 2 표시 ──
QUESTION_2 = """
다음 문장에서 틀린 부분을 찾아 적절하게 고치시오. "극성 공유 결합만 존재하는 분자는 극성 분자이다."
"""
...
# ── 0. OpenAI 클라이언트 ──
try:
    client = OpenAI(api_key="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")
except KeyError:
...
# ── 1. 문항별 채점 기준 (교사가 수정) ──
GRADING_GUIDELINES = {
    1: "같은 족 내에서는 원자번호와 전기음성도가 반비례한다.",
    2: "극성 공유 결합만 존재하는 분자도 구조가 대칭적이어서 쌍극자 모멘트가 0이라면 무극성 분자가 될 수 있다.",
    3: "평면 삼각형 구조이다."
}
